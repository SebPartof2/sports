...
export async function fetchTodaysGames(): Promise<Game[]> {
  try {
    // Step 1: Get today's schedule with cache busting
    // Note: Using 2025-10-02 since that's when the playoff games are happening
    const today = '2025-10-02' // Fixed date for the playoff games
    const cacheBuster = Date.now()
    
    console.log('Fetching games for date:', today)
    
    const scheduleResponse = await fetch(`https://statsapi.mlb.com/api/v1/schedule?sportId=1&date=${today}&_=${cacheBuster}`)
